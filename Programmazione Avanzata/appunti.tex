...
Ci sono vari tipi numerici. Anche i primitivi sono oggetti istanze di classi. 
...
\end{itemize}
Posso rappresentare qualsiasi intero fino a infinito. I float sono precisi fino a 15 cifre decimali. Qui ci sono alcuni operatori:
\begin{itemize}
	\item +, -
	\item Altri da completare
\end{itemize}
...
\end{itemize}
Posso usare più o meno le stesse operazioni sulle liste, apparte quelle che apportano modifiche. Posso usare per assegnazioni multiple a variabili (magari metto codice).

\subsubsection{Sets (insiemi)}
Struttura dati non ordinata con elementi univoci. Le creo come una lista ma con le graffe. Si crea set vuoto con \texttt{set()}. Posso anche crearli da una lista.
Operatori:
\begin{itemize}
...
Insieme di coppie chiave-valore. La sintassi è praticamente quella del JSON. Sono praticamente delle HashMap. Il dizionario vuoto si scrive con \texttt{{}}. Sono disponibili i classici del ecc.
...
Stringhe di formattazione, meglio che guardo la doc. \E simile a js e php, ma tipo printf del C. Esiste anche il comando Template. Ci sono un'infinità di operazioni.
